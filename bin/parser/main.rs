use mini_scheme::{Expr, Lexer, Parser};
use std::env;
use std::fs;
...
fn main() {
    ...
    match args.len() {
        1 => {
            ...
            println!(
                "Commands: :pretty (pretty print AST), :debug (debug AST), :tokens (show tokens too)"
            );
            parser_repl();
        }
        2 => {
...
fn format_expr_pretty(expr: &Expr, indent: usize) -> String {
    match expr {
        ...
        Expr::Boolean(b, _) => format!("Boolean({})", b),
        ...
        Expr::Define(def) => {
            format!(
                ...
                def.name,
                " ".repeat(indent + 2),
                format_expr_pretty(&def.value, indent + 2),
                " ".repeat(indent)
            )
        }
        Expr::Lambda(lambda) => {
        ...
        Expr::If(if_expr) => {
            format!(
                "If(\n{}condition: {},\n{}then: {},\n{}else: {}\n{})",
                " ".repeat(indent + 2),
                format_expr_pretty(&if_expr.condition, indent + 2),
                " ".repeat(indent + 2),
                format_expr_pretty(&if_expr.then_expr, indent + 2),
                " ".repeat(indent + 2),
                format_expr_pretty(&if_expr.else_expr, indent + 2),
                " ".repeat(indent)
            )
        }
        Expr::Let(let_expr) => {
            let bindings = let_expr
                ...
                .iter()
                .map(|(name, value)| {
                    format!("(\"{}\", {})", name, format_expr_pretty(value, indent + 4))
                })
                .collect::<Vec<_>>()
                .join(&format!(",\n{}", " ".repeat(indent + 4)));
            format!(
